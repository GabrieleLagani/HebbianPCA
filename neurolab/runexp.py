...
def run_experiment(config, mode, seeds, tokens, hpsearch, hpseeds, checkpoint, restart, branch):
	...
	if not hpsearch or mode == P.MODE_TST:
		for i in range(len(seeds)):
			print("\n**** ITERATION ID: " + str(seeds[i]) + " ****\n")
			# Prepare configuration
			CONFIG_OPTIONS = utils.retrieve(config)
			curr_config = utils.Config(config_id=config, config_options=CONFIG_OPTIONS, mode=mode,
			...
			if branch is not None and branch != curr_config.CONFIG_ID:
				...
				branch_config = utils.Config(config_id=branch, config_options=BRANCH_CONFIG_OPTIONS, mode=mode,
...
def main():
	...
	parser.add_argument('--mode', default=P.DEFAULT_MODE, choices=[P.MODE_TRN, P.MODE_TST], help="Whether you want to run a train or test experiment.")
