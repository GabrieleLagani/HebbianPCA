...
class VisionExperiment(Experiment):
	...
	def process_batch(self, batch):
		...
		for i in range(len(self.pre_net_list)): inputs = self.pre_net_list[i](inputs)[self.config.CONFIG_OPTIONS[P.KEY_PRE_NET_OUTPUTS][i]]
		...
		for i in range(len(self.net_list)):
			...
			# Forward step. Take the output from the last layer (class scores)
			outputs = self.net_list[i](outputs if i > 0 else inputs) # Let the network process the input
			...
			if i < len(self.net_list) - 1 or ( i == len(self.net_list) - 1 and self.config.CONFIG_OPTIONS.get(P.KEY_NET_OUTPUTS, None) is not None and i < len(self.config.CONFIG_OPTIONS[P.KEY_NET_OUTPUTS])):
				which_output = self.config.CONFIG_OPTIONS[P.KEY_NET_OUTPUTS][i]
				outputs = {key: outputs[key] for key in which_output.split('+')} if '+' in which_output else outputs[which_output]
...
class AEVisionExperiment(VisionExperiment):
	def process_batch(self, batch):
		inputs, _ = batch
		return super().process_batch((inputs, inputs))