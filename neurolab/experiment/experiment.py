...
class Experiment:
	...
	def load_state_dict(self, d):
		...
		for i in range(len(self.net_list)): self.net_list[i].load_state_dict(d['net_list'][i])
	...
	def load_models(self):
		...
		pre_net_outputs = self.config.CONFIG_OPTIONS.get(P.KEY_PRE_NET_OUTPUTS, None)
		if pre_net_modules is not None:
			for i in range(len(pre_net_modules)):
				...
				pre_net_list[i].eval()
		input_shape = self.config.CONFIG_OPTIONS.get(P.KEY_INPUT_SHAPE, None)
		if len(pre_net_list) > 0: input_shape = pre_net_list[len(pre_net_list) - 1].get_output_fmap_shape(pre_net_outputs[len(pre_net_list) - 1], fwd=True)
		net_list = []
		...
		net_outputs = self.config.CONFIG_OPTIONS.get(P.KEY_NET_OUTPUTS, None)
		for i in range(len(net_modules)):
			# Load network models
			if i > 0: input_shape = net_list[i - 1].get_output_fmap_shape(net_outputs[i - 1], fwd=True)
	...
	def save_results(self):
		...
		if utils.is_better(curr_res, self.best_result, P.HIGHER_IS_BETTER):
			...
			for i in range(len(self.net_list)):
				self.best_model_dicts[i] = self.net_list[i].state_dict()
